...
/*
    public static String isSortedAndHow(int[] array) {
        // Проверка на сортировку по возрастанию (нестрогая)
        if(IntStream.range(0, array.length-1)  // Создаем поток индексов от 0 до предпоследнего
                .allMatch(i -> array[i] <= array[i+1]))  // Проверяем, что каждый элемент <= следующего
            return "yes, ascending";  // Если условие выполняется для всех пар - возвращаем "возрастание"

        // Проверка на сортировку по убыванию (нестрогая)
        if(IntStream.range(0, array.length-1)  // Аналогично создаем поток индексов
                .allMatch(i -> array[i] >= array[i+1]))  // Проверяем, что каждый элемент >= следующего
            return "yes, descending";  // Если условие выполняется для всех пар - возвращаем "убывание"

        // Если ни одно из условий не выполнилось
        return "no";  // Массив не отсортирован ни по возрастанию, ни по убыванию
    }
*/
